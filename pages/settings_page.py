...
class SettingsPage(ctk.CTkFrame):
 ...
 text_color="#000000",).pack(side="left")
    ...
    def _build_system(self):
        ...
        # ── 天气城市设置 ──
        ctk.CTkLabel(card, text="🌤️ 天气预报城市",
                     font=ctk.CTkFont(family="Microsoft YaHei", size=16, weight="bold"),
                     text_color=self.C["text"]).pack(anchor="w", padx=20, pady=(8, 8))

        ctk.CTkLabel(card,
                     text="在看板页面显示天气预报的城市（最多4个，输入城市名称后自动查询）",
                     font=ctk.CTkFont(family="Microsoft YaHei", size=12),
                     text_color=self.C["text_secondary"]).pack(anchor="w", padx=20, pady=(0, 8))

        # 读取已保存的城市
        from pages.dashboard_page import get_weather_cities, BUILTIN_CITY_IDS, save_weather_cities, _get_city_id
        saved_cities = get_weather_cities()

        self._weather_city_vars = []
        self._weather_city_status = []
        city_frame = ctk.CTkFrame(card, fg_color="transparent")
        city_frame.pack(fill="x", padx=20, pady=(0, 8))

        for i in range(4):
            row = ctk.CTkFrame(city_frame, fg_color="transparent")
            row.pack(fill="x", pady=3)

            ctk.CTkLabel(
                row, text=f"城市{i+1}:", width=50,
                font=ctk.CTkFont(family="Microsoft YaHei", size=12),
                text_color=self.C["text"],
            ).pack(side="left")

            default_name = saved_cities[i]["name"] if i < len(saved_cities) else ""
            var = tk.StringVar(value=default_name)
            self._weather_city_vars.append(var)

            entry = ctk.CTkEntry(
                row, textvariable=var, width=140, height=28,
                font=ctk.CTkFont(family="Microsoft YaHei", size=12),
                fg_color="#FFFFFF", text_color="#000000",
                border_color="#E2E0DA", border_width=1,
                corner_radius=6,
            )
            ...
            self._weather_city_status.append(status_label)
        ...
        detect_btn = ctk.CTkButton(
            ...
            width=140, height=28,
            font=ctk.CTkFont(family="Microsoft YaHei", size=12),
            fg_color=self.C["primary_light"], hover_color=self.C["primary"],
            corner_radius=6, border_width=1,
            border_color=self.C["primary"],
            command=self._detect_weather_cities,
            text_color="#000000",
        )
        detect_btn.pack(anchor="w", padx=20, pady=(0, 8))
    ...
    def _detect_weather_cities(self):
        """自动检测天气城市ID"""
        from pages.dashboard_page import BUILTIN_CITY_IDS, _get_city_id
        import threading

        def _worker():
            results = []
            for i, var in enumerate(self._weather_city_vars):
                name = var.get().strip()
                if not name:
                    results.append((i, "", ""))
                    continue
                if name in BUILTIN_CITY_IDS:
                    results.append((i, name, BUILTIN_CITY_IDS[name]))
                else:
                    cid = _get_city_id(name)
                    if cid:
                        results.append((i, name, cid))
                    else:
                        results.append((i, name, ""))

            def _update_ui():
                for idx, name, cid in results:
                    if cid:
                        self._weather_city_status[idx].configure(
                            text=f"✅ {cid}", text_color="#8FA882"
                        )
                    elif name:
                        self._weather_city_status[idx].configure(
                            text="❌ 未找到", text_color="#B56A6A"
                        )
                    else:
                        self._weather_city_status[idx].configure(
                            text="", text_color=self.C["text_secondary"]
                        )
 ...
 text_color="#000000",).pack(side="left", padx=4)
    ...
    def _save_all(self):
        ...
        # 保存天气城市配置
        if hasattr(self, '_weather_city_vars'):
            from pages.dashboard_page import save_weather_cities, BUILTIN_CITY_IDS, _get_city_id
            cities = []
            for var in self._weather_city_vars:
                name = var.get().strip()
                if name:
                    cid = BUILTIN_CITY_IDS.get(name, "") or _get_city_id(name) or ""
                    cities.append({"name": name, "id": cid})
            if cities:
                save_weather_cities(cities)
