...
class PackagingPage(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # ── 筛选栏 ──
        filter_bar = ctk.CTkFrame(self, fg_color=self.C["card"], corner_radius=8, height=44)
        ...
        self.tree.column("物料名称", width=140, anchor="w")
        self.tree.column("项目号", width=80)
        self.tree.column("下单数量", width=80)
        self.tree.column("下单厂家", width=100)
        self.tree.column("所属项目", width=100)
        self.tree.column("比价单价", width=80)
        self.tree.column("合同状态", width=80)
        self.tree.column("通知日期", width=90)
        self.tree.column("预计发货", width=90)
        self.tree.column("发货日期", width=90)
        self.tree.column("预计到货", width=90)
        self.tree.column("状态", width=70)
        self.tree.column("操作", width=90, anchor="center")
    ...
    def _on_row_click(self, event):
        ...
        if col_idx != 11:  # 操作列是第12列（0-indexed: 11）
            return
    ...
    def _toggle_archive(self):
        ...
        self.archive_btn.configure(
            text="📋 查看进行中" if self.show_archived else "📁 查看归档"
        )
        self._load_data()
...
class PackagingForm(ctk.CTkToplevel):
    ...
    def _build_section(self, parent, title, builder):
        frame = ctk.CTkFrame(parent, fg_color=self.C["card"], corner_radius=10)
