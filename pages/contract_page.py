...
class SupplierDialog(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        ctk.CTkButton(bf,text="保存",width=100,height=32,
            font=ctk.CTkFont(size=14),command=self._save, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="right",padx=4)
        ctk.CTkButton(bf,text="取消",width=100,height=32,
            fg_color="transparent",hover_color="#4B5563",
            font=ctk.CTkFont(size=14),command=self.destroy, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="right",padx=4)
    def _save(self):
...
class PartyAConfigDialog(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        ctk.CTkButton(bf,text="保存",width=100,height=34,
            ...
            command=self._save, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="right",padx=4)
...
class ContractPage(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        toolbar.pack(fill="x",padx=20,pady=(12,8)); toolbar.pack_propagate(False)
        bf=ctk.CTkFrame(toolbar,fg_color="transparent"); bf.pack(side="left",pady=5)
        ctk.CTkButton(bf,text="供方",width=80,height=34,
            fg_color="transparent",hover_color="#9B4F4A",
            font=ctk.CTkFont(size=13,weight="bold"),
            command=self._manage_suppliers, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="left",padx=4)
        ctk.CTkButton(bf,text="需方",width=80,height=34,
            fg_color="transparent",hover_color="#1D4ED8",
            font=ctk.CTkFont(size=13,weight="bold"),
            command=self._open_party_a_config, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="left",padx=4)
        ctk.CTkButton(bf,text="生成",width=80,height=34,
            fg_color="transparent",hover_color=self.C["primary_hover"],
            font=ctk.CTkFont(size=13,weight="bold"),
            command=self._generate_contract, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="left",padx=4)
        ...
        ctk.CTkButton(sb,text="检索",width=60,height=30,
            ...
            command=lambda:self._on_search(None), corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="left",padx=(0,10),pady=12)
        # 确认按钮
        self.confirm_btn = ctk.CTkButton(sb,text="确认",width=100,height=30,
            fg_color="transparent",hover_color="#7A9A6E",
            font=ctk.CTkFont(size=13,weight="bold"),
            command=self._on_confirm_supplier, corner_radius=8, border_width=1, border_color="#8B7D6B")
        self.confirm_btn.pack(side="left",padx=(0,10),pady=12)
        ctk.CTkButton(sb,text="重置",width=100,height=30,
            fg_color="transparent",hover_color="#4B5563",
            font=ctk.CTkFont(size=13,weight="bold"),
            command=self._on_reset_all, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="left",padx=(0,10),pady=12)
        # 确认状态标签
        ...
        scroll=ctk.CTkScrollableFrame(self,fg_color="transparent",corner_radius=0,
            ...
            scrollbar_button_hover_color=self.C.get("sidebar_hover","#ddd"))
    ...
    def _manage_suppliers(self):
        ...
        dialog.title("供应商管理"); dialog.geometry("700x520")
        ...
        ctk.CTkButton(top,text="+ 新增",width=80,height=30,
            font=ctk.CTkFont(size=13),
            command=lambda:self._add_supplier_dialog(dialog), corner_radius=8,
            border_width=1, border_color="#8B7D6B",
 text_color="#000000",).pack(side="right",padx=4,pady=8)
        lf=WheelScrollFrame(dialog,fg_color=self.C["bg"])
        lf.pack(fill="both",expand=True,padx=16,pady=(0,12))
        self._refresh_supplier_list(dialog,lf)
    def _refresh_supplier_list(self,dialog,lf):
        inner = lf._inner
        for w in inner.winfo_children(): w.destroy()
        if not self.suppliers:
            ctk.CTkLabel(inner,text="暂无供应商数据，点击右上角 [新增] 添加",
                font=ctk.CTkFont(size=14),text_color="#9CA3AF").pack(pady=24)
        else:
            for s in self.suppliers:
                r=ctk.CTkFrame(inner,fg_color=self.C["card"],corner_radius=6)
                ...
                ctk.CTkButton(r,text="编辑",width=50,height=26,
                    ...
                    command=lambda sup=s: [dialog.destroy(),self._edit_supplier_dialog(sup)], corner_radius=8,
                    border_width=1, border_color="#8B7D6B",
 text_color="#000000",).pack(side="right",padx=4,pady=4)
                ctk.CTkButton(r,text="删除",width=50,height=26,
                    ...
                    font=ctk.CTkFont(size=12),
                    command=lambda sup=s: self._delete_supplier(sup,dialog,lf), corner_radius=8,
                    border_width=1, border_color="#8B7D6B",
 text_color="#000000",).pack(side="right",padx=2,pady=4)
    ...
    def _build_products(self,parent):
        ...
        ctk.CTkButton(hf,text="+ 添加产品",width=100,height=30,
            fg_color="transparent",hover_color="#7A9A6E",
            font=ctk.CTkFont(size=13),
            command=self._add_product_row, corner_radius=8, border_width=1, border_color="#8B7D6B", text_color="#000000",).pack(side="right")
        self.product_container=ctk.CTkFrame(card,fg_color="transparent")
        self.product_container.pack(fill="x",padx=12,pady=(0,14))
        hdr=ctk.CTkFrame(self.product_container,fg_color="transparent")
        hdr.pack(fill="x",pady=(0,4))
        hdr_widths={"物料名称":260,"项目号":78,"材料结构":130,"尺寸":130,
                     "单位":65,"数量":65,"单价":65,"金额":65,"备注":130,"操作":80}
        for h in ["物料名称","项目号","材料结构","尺寸","单位","数量","单价","金额","备注","操作"]:
            ctk.CTkLabel(hdr,text=h,width=hdr_widths.get(h,88),anchor="center",
    ...
    def _add_product_row(self,default=False):
        ...
        entries={}
        entry_w={"物料名称":260,"项目号":78,"材料结构":130,"尺寸":130,
                  "单位":65,"数量":65,"单价":65}
        for fn in ["物料名称","项目号","材料结构","尺寸","单位","数量","单价"]:
            e=ctk.CTkEntry(rf,width=entry_w[fn],height=30,font=ctk.CTkFont(size=12),
                           justify="center")
            e.pack(side="left",padx=1); entries[fn]=e
        amt_label=ctk.CTkLabel(rf,text="",width=65,anchor="center",
        ...
        del_btn=ctk.CTkButton(rf,text="X",width=30,height=30,
            fg_color="transparent",hover_color="#A85A5A",
            font=ctk.CTkFont(size=12),
            command=lambda f=rf: self._del_product_row(f), corner_radius=8, border_width=1, border_color="#8B7D6B")
