...
class Database:
    ...
    def _init_tables(self):
        ...
        """)
        # 迁移：为旧表添加新列（如果不存在）
        for col in ['price_tier', 'price1_tier', 'price2_tier', 'price3_tier']:
            try:
                c.execute(f"ALTER TABLE third_party_records ADD COLUMN {col} TEXT DEFAULT ''")
            except Exception:
                pass  # 列已存在
    ...
    def _migrate_tables(self):
        ...
        # plan_records — 添加缺失列
        c.execute("PRAGMA table_info(plan_records)")
        ...
        for col_name, col_type in [
            ...
            if col_name not in pr_cols:
                try:
                    c.execute(f"ALTER TABLE plan_records ADD COLUMN {col_name} {col_type}")
                except Exception:
                    pass
    ...
    # ====== 采购计划 (V2.3.2 新增) ======
    def save_plan_record(self, data):
        """新增一条计划记录"""
        c = self.conn.cursor()
        c.execute("""
            INSERT INTO plan_records(approval_no, item_seq, material_name, spec, quantity, unit,
                unit_price, amount, expected_delivery, remark, submitted_at, approved_at)
            VALUES(:approval_no, :item_seq, :material_name, :spec, :quantity, :unit,
                :unit_price, :amount, :expected_delivery, :remark, :submitted_at, :approved_at)
        """, data)
        self.conn.commit()
        return c.lastrowid

    def update_plan_record(self, pid, data):
        """更新计划记录"""
        c = self.conn.cursor()
        data["id"] = pid
        c.execute("""
            UPDATE plan_records SET approval_no=:approval_no,
                item_seq=:item_seq,
    ...
    def get_plan_records(self, archived=0):
        """获取计划记录（默认未归档）"""
        c = self.conn.cursor()
        c.execute("SELECT * FROM plan_records WHERE archived=? ORDER BY id DESC", (archived,))
        return [dict(r) for r in c.fetchall()]

    def get_plan_record(self, pid):
        """获取单条计划记录"""
        c = self.conn.cursor()
        c.execute("SELECT * FROM plan_records WHERE id=?", (pid,))
        row = c.fetchone()
        return dict(row) if row else None

    def archive_plan_record(self, pid):
        """归档计划记录"""
        c = self.conn.cursor()
        c.execute("UPDATE plan_records SET archived=1 WHERE id=?", (pid,))
        self.conn.commit()

    def delete_plan_record(self, pid):
        """删除计划记录"""
        c = self.conn.cursor()
        c.execute("DELETE FROM plan_records WHERE id=?", (pid,))
        self.conn.commit()
    ...
    # ====== 三方比价记录 (V1.9.8 新增) ======
    def get_third_party_records(self):
        c = self.conn.cursor()
        c.execute("SELECT * FROM third_party_records ORDER BY id DESC")
        return [dict(r) for r in c.fetchall()]

    def get_third_party_record(self, oid):
        c = self.conn.cursor()
        c.execute("SELECT * FROM third_party_records WHERE id=?", (oid,))
        row = c.fetchone()
        return dict(row) if row else None

    def save_third_party_record(self, data):
        c = self.conn.cursor()
        c.execute("""
            INSERT INTO third_party_records(product_name, item_no,
                material_structure, spec_size, quantity_tier,
                supplier1, supplier2, supplier3, final_supplier,
                price1_tier, price2_tier, price3_tier)
            VALUES(:product_name, :item_no,
                :material_structure, :spec_size, :quantity_tier,
                :supplier1, :supplier2, :supplier3, :final_supplier,
                :price1_tier, :price2_tier, :price3_tier)
        """, data)
    ...
    def update_third_party_record(self, oid, data):
        c = self.conn.cursor()
        data["id"] = oid
        c.execute("""
            UPDATE third_party_records SET product_name=:product_name,
                item_no=:item_no, material_structure=:material_structure,
                spec_size=:spec_size, quantity_tier=:quantity_tier,
                supplier1=:supplier1, supplier2=:supplier2,
                supplier3=:supplier3, final_supplier=:final_supplier,
                price1_tier=:price1_tier, price2_tier=:price2_tier,
                price3_tier=:price3_tier,
                updated_at=datetime('now','localtime')
            WHERE id=:id
        """, data)
        self.conn.commit()

    def delete_third_party_record(self, oid):
        c = self.conn.cursor()
        c.execute("DELETE FROM third_party_records WHERE id=?", (oid,))
        self.conn.commit()
    ...
    # ====== 成品BOM CRUD (V1.9.7 从 BOM管理系统 集成) ======

    @staticmethod
    def _clean_bom_value(val, val_type="str"):
        """清洗单个值：将 'null'/'None'/'' 转为合理的默认值"""
        if val is None:
            return "" if val_type == "str" else (0.0 if val_type == "float" else 0)
        s = str(val).strip()
    ...
    def save_product_bom(self, data):
        cleaned = {
            "finished_project_no": self._clean_bom_value(data.get("finished_project_no", ""), "str") or "",
            "product_name": self._clean_bom_value(data.get("product_name", ""), "str"),
            "spec": self._clean_bom_value(data.get("spec", ""), "str"),
            "retail_price": self._clean_bom_value(data.get("retail_price", 0), "float"),
            "brand": self._clean_bom_value(data.get("brand", ""), "str"),
            "material_project_no": self._clean_bom_value(data.get("material_project_no", ""), "str") or "",
            "material_name": self._clean_bom_value(data.get("material_name", ""), "str"),
            "quantity": self._clean_bom_value(data.get("quantity", 0), "float"),
            "unit": self._clean_bom_value(data.get("unit", ""), "str"),
        }
        c = self.conn.cursor()
        c.execute("""
    ...
    def import_product_bom(self, rows):
        ...
        for row in rows:
            cleaned = {
                "finished_project_no": self._clean_bom_value(row.get("finished_project_no", ""), "str") or "",
                "product_name": self._clean_bom_value(row.get("product_name", ""), "str"),
                "spec": self._clean_bom_value(row.get("spec", ""), "str"),
                "retail_price": self._clean_bom_value(row.get("retail_price", 0), "float"),
                "brand": self._clean_bom_value(row.get("brand", ""), "str"),
                "material_project_no": self._clean_bom_value(row.get("material_project_no", ""), "str") or "",
                "material_name": self._clean_bom_value(row.get("material_name", ""), "str"),
                "quantity": self._clean_bom_value(row.get("quantity", 0), "float"),
                "unit": self._clean_bom_value(row.get("unit", ""), "str"),
            }
    ...
    def save_product_bom_batch(self, rows):
        """批量保存 BOM（V2.2.7 新增多物料新增）"""
        c = self.conn.cursor()
        for row in rows:
            cleaned = {
                "finished_project_no": self._clean_bom_value(row.get("finished_project_no", ""), "str") or "",
